...
const session = {
  ...
  cameraMode: localStorage.getItem("verite.camera") || localStorage.getItem("impact.camera") || "side",
...
function logEvent(type, detail = {}) {
  const event = { sessionId: session.id, v: "verite", t: Math.round(performance.now()), type, detail };
  hud.pushEvent(event);
  session.pendingLogs.push(event);
  console.log("[verite]", event);
}
  ...
  switch (type) {
    ...
    case "signature": {
      ...
      if (type === "takedown") {
        ...
        // Ordinary contact: the attacker visually closes the last gap so the
        // technique lands ON the body, with a hot flash at the contact point.
        const gap = Math.abs(rigOf(payload.winner).x - rigOf(payload.loser).x);
        rigOf(payload.winner).react("lunge", { dist: Math.max(0, gap - 0.95) });
...
addEventListener("keydown", event => {
  ...
  if (key === "c" && session.config.mode !== "pvp") {
    ...
    localStorage.setItem("verite.camera", session.cameraMode);
...
function frame(now) {
  ...
  if (combat && session.mode === "fight") {
    ...
    const rigDt = combat.game.hitStop > 0 && !seqLive ? 0 : dtRaw;
