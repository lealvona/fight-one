...
const MOVE_POSES = {
  ...
  HB: {
    ...
    extend: {
      ...
      root: [0, -0.08, 0.16, 0.18, 0, 0], kneeL: [0.45, 0, 0], kneeR: [0.45, 0, 0]
    }
  }
};
...
export function createRig(char, side) {
  ...
  const pelvisColor = char.costume.trunks ? c.trunks : char.costume.gi ? c.gi : char.costume.tunic ? c.legs : char.costume.bodysuit ? c.suit : char.costume.armor ? c.under : (c.trousers || c.tights || c.suit || 0x222222);
  const legColor = char.costume.trunks ? c.skin : c.trousers || c.tights || (char.costume.bodysuit ? c.suit : char.costume.armor ? c.under : char.costume.gi ? c.giShade : c.legs || 0x202020);
  ...
  function arm(prefix, dir) {
    ...
    sh.add(upper);
    if (char.costume.armbands) {
      const band = capsule(H * 0.042 * bulk, H * 0.045, mat(c.bands, { rough: 0.55 }));
  ...
  function buildCostume() {
    ...
    if (co.trunks) {
      for (const dir of [1, -1]) {
        const hem = capsule(H * 0.055 * bulk, H * 0.07, mat(c.trunks));
        hem.position.set(dir * dims.hipX, -H * 0.06, 0);
        hips.add(hem);
      }
    }
    if (co.sash) {
