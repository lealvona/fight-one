...
function logEvent(type, detail = {}) {
  const event = { sessionId: session.id, v: "apex", t: Math.round(performance.now()), type, detail };
  hud.pushEvent(event);
  session.pendingLogs.push(event);
  console.log("[apex]", event);
}
...
function onEffect(type, payload) {
  ...
  switch (type) {
    ...
    case "signature": {
      ...
      if (payload.move.super && type === "hit") {
      ...
      if (payload.launched) stage.spawnFloater(pos, "LAUNCHED", color);
      ...
      if (type === "hit" && !payload.move.super) audio.thud(payload.damage / 10);
      else audio.heavy();
      break;
    }
    case "burst": {
...
function frame(now) {
  ...
  if (combat && session.mode === "fight") {
    ...
    const px = -sep / 2;
    const ex = sep / 2;
    const rigDt = combat.game.hitStop > 0 ? 0 : dtRaw;
    ...
    session.stage.updateCamera(dtRaw, combat.game, (px + ex) / 2, sep, now);
    hud.update(combat, session.config, session.gauntlet);
  } else {
    session.timeScale = 1;
    if (session.stage) session.stage.updateCamera(dtRaw, { slowMo: 0, shake: 0 }, 0, 2.4, now);
  }
