...
export function createCombat({ chars, log, effect, roundTime = 60, winsNeeded = 2, maxRounds = 3, seed = 0 }) {
  ...
  function makeActor(id, side, char) {
    return {
      ...
      lastHitMove: null,
      stringCount: 0,
      stats: { exchanges: 0, redirects: 0, bursts: 0, takedowns: 0, maxString: 1 }
    };
  }
  ...
  function intent(actorId, key, now) {
    ...
    if (key === "g") {
      ...
      move = resolveMove(actor.char, key, actor.flowState);
  ...
  function enqueue(actor, move, now, human) {
    ...
    const speed = actor.char.stats.speed * (actor.flowState ? 1.06 : 1);
    // Cadence reward: a fighter in rhythm starts techniques sooner.
    const rhythm = 1 - Math.min(0.09, actor.tempo * 0.06);
    const item = {
      ...move,
      id: `${now.toFixed(0)}-${Math.random().toString(36).slice(2, 7)}`,
      startup: Math.round((move.startup * rhythm) / speed),
  ...
  function tickActor(actor, dt) {
    ...
    if (actor.downTime > 0) {
      ...
      const armed = actor.queue.find(q => q.ground);
      if (armed && !actor.koed && actor.downTime > 150) {
        const foe = other(actor);
        if (foe.downTime <= 0 && foe.staggerTime <= 0 && foe.hp > 0 && game.range < 0.52) {
          actor.queue = [];
          executeGroundReversal(actor, foe);
        }
      } else if (actor.downTime <= 0) {
        actor.queue = actor.queue.filter(q => !q.ground); // grip expires on getting up
      }
      return;
    }
    ...
    if (!actor.current && actor.queue.length) {
      ...
      if (actor.current.art) {
        game.hitStop = Math.max(game.hitStop, 220);
        effect("artFlash", { actor: actor.id, move: actor.current });
      }
  ...
  function applyWin(winner, loser, winMove, loseMove) {
    ...
    // Combo strings: distinct techniques landed in rhythm hit harder.
    const tNow = performance.now();
    if (tNow - winner.lastHitAt < 1500 && winner.lastHitMove !== winMove.name) winner.stringCount += 1;
    else winner.stringCount = 1;
    winner.lastHitAt = tNow;
    winner.lastHitMove = winMove.name;
    winner.stats.maxString = Math.max(winner.stats.maxString, winner.stringCount);
    loser.stringCount = 0;
    loser.lastHitAt = 0;
    ...
    if (winner.stringCount > 1) damage *= 1 + Math.min(0.3, (winner.stringCount - 1) * 0.1);
    ...
    game.hitStop = Math.max(game.hitStop, Math.min(180, 60 + damage * 5));
    winner.stats.exchanges += 1;

    let kind = "hit";
    let launched = false;
    if (takedown) {
      kind = "takedown";
      winner.stats.takedowns += 1;
      knockdown(loser, 900);
      ...
      spendFlow(winner);
    } else if (winMove.knockdownOnHit) {
      // Cinema-grade launcher: the super version sends them flying.
      launched = true;
      knockdown(loser, 850);
      game.slowMo = 380; game.slowMoScale = 0.5;
      game.callout = `${winMove.name} launches ${pretty(loser)}`;
      addFlow(winner, 12, "launcher");
    } else {
    ...
    effect(kind, {
      ...
      string: winner.stringCount, launched
    });
    log("exchange", {
  ...
  function applyDefense(defender, attacker, defenseMove, attackMove) {
    ...
    if (justGuard && attackMove.family === "strike") {
      ...
      defender.stats.bursts += 1;
      game.hitStop = Math.max(game.hitStop, 80);
  ...
  function executeGroundReversal(actor, foe) {
    const damage = Math.max(2, Math.round(GROUND_REVERSAL.damage * actor.char.stats.power));
    foe.hp = clamp(foe.hp - damage, 0, 100);
    foe.posture = clamp(foe.posture - GROUND_REVERSAL.posture, 0, foe.postureMax);
    foe.hitPulse = 220;
    knockdown(foe, 1500);
    actor.downTime = Math.min(actor.downTime, 500);
    actor.stats.redirects += 1;
    addFlow(actor, 12, "groundReversal");
    game.range = clamp(game.range + 0.1, 0.08, 0.96);
    game.shake = Math.min(12, game.shake + 6);
    game.callout = `${pretty(actor)} reverses from the floor`;
    effect("groundReversal", { actor: actor.id, target: foe.id, damage });
    log("groundReversal", { actor: actor.id, target: foe.id, damage });
    if (foe.hp <= 0) koFinish(actor, foe, GROUND_REVERSAL);
  }
  ...
  function knockdown(actor, ms) {
    ...
    actor.stringCount = 0;
    actor.lastHitAt = 0;
    // Cadence retention: a slip means nothing on the floor, but a queued
    // clinch survives the slam as an armed ground reversal.
    const keptGrip = actor.queue.some(q => q.family === "clinch" && !q.free && !q.ground);
    actor.queue = [];
    if (keptGrip && !actor.koed) {
      actor.queue.push({
        ...GROUND_REVERSAL,
        id: `ground-${Math.random().toString(36).slice(2, 7)}`,
        age: 0, tempo: actor.tempo, resolved: false
      });
      game.callout = `${pretty(actor)} keeps the grip going down`;
      effect("groundArmed", { actor: actor.id });
      log("groundArmed", { actor: actor.id });
    }
    actor.current = null;
    actor.phaseTime = 0;
  ...
  function regenerate(dt) {
    for (const actor of Object.values(actors)) {
      ...
      if (actor.flowState) {
        actor.flow -= 0.009 * dt; // the cinema window burns down
        if (actor.flow <= 0) {
          actor.flow = 0;
          actor.flowState = false;
          effect("flowEnd", { actor: actor.id });
        }
      } else if (actor.flow > 0) {
        actor.flow = Math.max(0, actor.flow - 0.0008 * dt);
      }
    }
  }
