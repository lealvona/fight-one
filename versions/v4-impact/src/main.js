...
const session = {
  ...
  cameraMode: localStorage.getItem("impact.camera") || "side",
...
function logEvent(type, detail = {}) {
  const event = { sessionId: session.id, v: "impact", t: Math.round(performance.now()), type, detail };
  hud.pushEvent(event);
...
function processSeqFx(now) {
  ...
  for (let i = session.seqFx.length - 1; i >= 0; i--) {
    ...
    if (fx.power >= 1.6) audio.heavy();
    else audio.thud(fx.power);
  }
}
  ...
  switch (type) {
    ...
    case "signature": {
      ...
      if (type === "takedown") {
        ...
        // Ordinary contact: flinch + sparks at the body, no choreography.
...
addEventListener("keydown", event => {
  ...
  if (key === "c" && session.config.mode !== "pvp") {
    ...
    localStorage.setItem("impact.camera", session.cameraMode);
